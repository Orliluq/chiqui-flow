...
```mermaid
flowchart TD
    A[Usuario] --> B[Interpretación de intención]
    B --> C[Router semántico]
    C --> D[RAG - Base de conocimiento]
    D --> E[Embeddings + búsqueda]
    E --> F[Context Builder]
    F --> G[LLM - Motor de razonamiento]
    G --> H[Formatter de respuesta]
    H --> I[Usuario]
```
